...
//    }
    ...
    public Solver(String[] args) throws IOException {
        if (args.length == 0) {
            ...
            System.out.println("Enter the location coordinates based on given coordinates by using enter each number");
            System.out.println("Valid coordinates taken are:");
    ...
    private void init(int row, int col) {
        startingBoard = new GameBoard(row, col);
    }

    //Depth First Search in a tree method
    public void DFS() {
        GameTree root = new GameTree(startingBoard);

        for (GameBoard nextBoard : startingBoard.possibleBoards()) {
