    ...
    public List<GameBoard> possibleBoards() {
        ...
        for (int i = 0; i < 5; ++i)
            for (int j = 0; j <= i; ++j) {
                ...
                List<Move> possibleMoves = Moves.getMoves(startPosition);
    ...
    public boolean finalBoard() {
        ...
        for (int i = 0; i < 5; ++i)
            for (int j = 0; j <= i; ++j)
                if (pins[i][j]) {
                    remainingLocations++;
                    if (remainingLocations > 1)	// optimize, early out, more than 1 pin is not final board
                        return false;
                }

        return remainingLocations == 1;
    }
